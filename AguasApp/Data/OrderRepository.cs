...
namespace AguasApp.Data
{
    public class OrderRepository : GenericRepository<Order>, IOrderRepository
    {
        ...
        public async Task AddItemToCartAsync(Product product, string userName) 
        {
            ...
            var productWeb = await _context.Products.FindAsync(product.Id);
            if (product == null)
            {
                return;
            }
            ...
            var orderDetailTemp = await _context.OrderDetailsTemp
                .Where(odt => odt.User.Id == user.Id && odt.Product.Id == productWeb.Id)
                .FirstOrDefaultAsync();
            ...
            {
                if (orderDetailTemp.Product == product)
                {
